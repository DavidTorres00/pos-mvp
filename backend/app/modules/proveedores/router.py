...
from app.schemas.proveedor import ProveedorCreate, ProveedorEstado, ProveedorResumenOut, ProveedorUpdate
...
@router.get("", response_model=Pagina[ProveedorResumenOut])
def listar(
    q: str | None = None,
    paginacion: ParametrosPaginacion = Depends(parametros_paginacion),
    db: Session = Depends(get_db),
) -> Pagina[ProveedorResumenOut]:
...
@router.post("", response_model=ProveedorResumenOut, status_code=status.HTTP_201_CREATED)
def crear(
...
@router.get("/{proveedor_id}", response_model=ProveedorResumenOut)
def obtener(proveedor_id: int, db: Session = Depends(get_db)) -> ProveedorResumenOut:
    try:
...
@router.put("/{proveedor_id}", response_model=ProveedorResumenOut)
def actualizar(proveedor_id: int, payload: ProveedorUpdate, db: Session = Depends(get_db)) -> ProveedorResumenOut:
    try:
...
@router.patch("/{proveedor_id}/estado", response_model=ProveedorResumenOut)
def cambiar_estado(
    proveedor_id: int,
