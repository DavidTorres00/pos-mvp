...
// Layout de pantalla completa (sin sidebar de la app) para los dos momentos en los que el
// usuario está fuera del flujo normal: login y apertura de caja. El panel izquierdo es
// puramente de marca — siempre el mismo, sin importar la pantalla ni el usuario — con tamaños
// fluidos (clamp) para aprovechar monitores grandes en vez de quedar topado a un ancho fijo.
// Lo que cambia por pantalla (acción, información del cajero) vive del lado derecho, vía `children`.
export function SplitBrandScreen({ footer, children }: SplitBrandScreenProps) {
  return (
    <div className="flex min-h-svh flex-col sm:flex-row">
      ...
      <div className="flex flex-1 items-center justify-center p-[clamp(2rem,5vw,6rem)]">{children}</div>
    </div>
  )
}
