...
export function useProductos(filtros: UseProductosFiltros, page = 1, size = 20) {
  const { q, activo, categoriaId } = filtros
  const sucursalId = useSucursalActivaStore((state) => state.sucursalId)
  return useQuery({
    queryKey: ['productos', q, activo, categoriaId, sucursalId, page, size],
    queryFn: () => listProductos({ q: q || undefined, activo, categoriaId, sucursalId, page, size }),
  })
}
