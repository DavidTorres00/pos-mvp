...
export function useProductos(q: string, page = 1, size = 20, activo?: boolean) {
  const sucursalId = useSucursalActivaStore((state) => state.sucursalId)
  return useQuery({
    queryKey: ['productos', q, activo, sucursalId, page, size],
    queryFn: () => listProductos({ q: q || undefined, activo, sucursalId, page, size }),
  })
}
