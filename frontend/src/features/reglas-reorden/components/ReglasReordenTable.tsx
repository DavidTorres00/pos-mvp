...
export function ReglasReordenTable({
  reglas,
  emptyMessage = 'No hay reglas de reorden configuradas.',
  onEdit,
  onToggleEstado,
}: ReglasReordenTableProps) {
            ...
            <TableRow key={regla.id}>
              ...
              <TableCell>
                <Switch
                  ...
                  aria-label={
                    regla.activo ? `Desactivar regla de ${regla.producto.nombre}` : `Activar regla de ${regla.producto.nombre}`
                  }
                />
              </TableCell>
              <TableCell>
      ...
      <AlertDialog open={pending !== null} onOpenChange={(open) => !open && setPending(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pending?.activo
                ? `¿Desactivar regla de ${pending?.producto.nombre}?`
                : `¿Activar regla de ${pending?.producto.nombre}?`}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {pending?.activo
