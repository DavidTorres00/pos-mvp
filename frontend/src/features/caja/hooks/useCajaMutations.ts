...
export function useAbrirCaja() {
  return useApiMutation((monto_inicial: number) => abrirCaja(monto_inicial), [['caja-actual']])
}

export function useCerrarCaja() {
  return useApiMutation((monto_final: number) => cerrarCaja(monto_final), [['caja-actual']])
}

export function useCrearMovimientoCaja() {
  return useApiMutation(
    (payload: MovimientoCajaPayload) => crearMovimientoCaja(payload),
    [['caja-actual'], ['caja-movimientos'], ['caja-resumen']],
  )
}
